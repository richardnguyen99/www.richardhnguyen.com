...
```html
<div
  class="bg-[#bada55] text-[22px] before:content-['Festivus'] grid grid-cols-[fit-content(--spacing(32))]"
>
  <!-- ... -->
</div>
```
...
```html
<div class="mask-type-luminance hover:mask-type-alpha block">
  <!-- ... -->
</div>
```
...
function App() {
  ...
  return (
    <main
      className="flex flex-col items-center gap-8 py-16 max-w-7xl mx-auto"
      style={
        {
    ...
    >
      ...
      <p className="absolute bottom-4 bg-amber-500 p-4 rounded-lg w-[calc(var(--container-size)-48px)] left-6 border border-yellow-700 text-amber-50">
