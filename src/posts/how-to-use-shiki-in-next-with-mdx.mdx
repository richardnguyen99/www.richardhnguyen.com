...
- Update `CustomMDX` function to be an `async` function and use `await` for
  `shikiHighlighter`.

Let's create a simple code block in the `index.mdx` file to test the syntax
highlighting:

~~~mdx title="how-to-use-shiki-in-next-with-mdx.mdx"
{/* Other content */}
```sh
npx create-next-app --example https://github.com/vercel/examples/tree/main/solutions/blog shiki-mdx-next-app
...
```tsx title="app/components/custom-pre.tsx"
...
```tsx title="app/components/copy-button.tsx"
"use client";
import * as React from "react";
...
feature is to use the `navigator.clipboard` API.
...
```css title="app/globals.css" displayLineNumbers="false"
.prose pre {
