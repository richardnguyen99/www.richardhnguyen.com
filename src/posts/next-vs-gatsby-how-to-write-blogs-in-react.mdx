...
[0;32m✔[0;0m Would you like to use [0;1;36mTypeScript[0;0m? … No / [0;4;34mYes[0;0m
[0;32m✔[0;0m Would you like to use [0;1;36mESLint[0;0m? … No / [0;4;34mYes[0;0m
[0;32m✔[0;0m Would you like to use [0;1;36mTailwind CSS[0;0m? … No / [0;4;34mYes[0;0m
[0;32m✔[0;0m Would you like your code inside a [0;1;36m`src/` directory[0;0m? … No / [0;4;34mYes[0;0m
[0;32m✔[0;0m Would you like to use [0;1;36mApp Router[0;0m? (recommended) … No / [0;4;34mYes[0;0m
[0;32m✔[0;0m Would you like to use [0;1;36mTurbopack[0;0m for `next dev`? … No / [0;4;34mYes[0;0m
[0;32m✔[0;0m Would you like to customize the [0;1;36mimport alias[0;0m (`@/*` by default)? … No / [0;4;34mYes[0;0m
[0;32m✔[0;0m What [0;1;36mimport alias[0;0m would you like configured? … [0;4;34m@/*[0;0m
...
[0;32m✔[0;0m · gatsby-mdx-blog-template
What would you like to name the folder where your site will be created?
[0;32m✔[0;0m ~/ gatsby-mdx-blog-template
[0;32m✔[0;0m Will you be using JavaScript or TypeScript?
[0;34m· TypeScript[0;0m
[0;32m✔[0;0m Will you be using a CMS?
[0;34m· No (or I'll add it later)[0;0m
[0;32m✔[0;0m Would you like to install a styling system?
[0;34m· Tailwind CSS[0;0m
[0;32m✔[0;0m Would you like to install additional features with other plugins?
[0;34m· Add responsive images[0;0m
[0;34m· Add an automatic sitemap[0;0m
[0;34m· Generate a manifest file[0;0m
[0;34m· Add Markdown and MDX support[0;0m


Thanks! Here's what we'll now do:

    🛠  Create a new Gatsby site in the folder [0;35mgatsby-mdx-blog-template[0;0m
    🎨 Get you set up to use [0;35mTailwind CSS[0;0m for styling your site
    🔌 Install [0;35mgatsby-plugin-image[0;0m, [0;35mgatsby-plugin-sitemap[0;0m, [0;35mgatsby-plugin-manifest[0;0m, [0;35mgatsby-plugin-mdx[0;0m
...
However, it's redundant and not scalable for reusable components. A better
...
NextJS and GatsbyJS have official guides and supports for TypeScript, from
writing React code to deeply interfering with the build process. However,
because GatsbyJS uses its own proprietary API files and GraphQL to fetch data,
it's more complicated to config and use TypeScript. Meanwhile, TypeScript does
not require to write NextJS-specific codes and configurations; you feel like
you are just writing TypeScript code with React components. For this criteria,
**using TypeScript in NextJS feels more natural and easier than in GatsbyJS**.
...
const PostLayout: React.FC<React.PropsWithChildren<Queries.PostLayoutQuery>> = ({ mdx, children }) => {
  return (
    <div className="min-h-screen max-w-3xl mx-auto pr-4 pl-8 font-(family-name:--font-geist-sans)">
...
you are writing GatsbyJS code, with GraphQL and its proprietary APIs. However,
...
whether your pages are rendered statically or on server-side. GatsbyJS does not
...
requests. However, NextJS has a more straightforward approach to handle search
...
metadata object, GatsbyJS uses a combination of configurations and components
to handle SEO.
...
normal React applications. These methods are native and official supported from
the frameworks with great documentation.
...
documentation on what fields, or how you should use it. If you want to enforce
...
Since we are building a blog, we will focus on those features that are mentioned
...
just for NextJS. The package lets you set up Cloudinary environment variables
and use the React component immediately.
...
Searching, or indexing, or autocomplete, is a crucial part of building a blog.
...
upload your content to Algolia using their write APIs. Algolia provides
...
> SSR to generate HTML from `react-instantsearch` components on the server.

`autocomplete-js` provides a pre-defined UI logic such as functionalities,
accessibility and keyboard navigation. However, you are in control of writing
your own UI components and integrating them with `autocomplete-js` props.
...
automatic process to upload the indexing data to Algolia. Meanwhile, the
configuration and setup in GatsbyJS is more complicated due to GraphQL queries.
...
supported in many cases andd well maintained to resolve many issues. I cannot
...
NextJS, on the other hand, provides a more detailed documentation and has an
