...
transfered by plain text and does not have any standard schema. The requirements
and the structure of the data should be agreed upon by the client and the
server.
...
With developement perspective, REST also doesn't have a type-checking mechanism
...
Let's take a look at a simple exaple of fetching authors and their songs in a
...
```sh displayLineNumbers="false"
curl -X POST http://localhost:5000/graphql \
  -H "Content-Type: application/json" \
  -d '{"query": "query AuthorQuery { author(id: 1) { id name songs { id title } } }"}'
```
...
developers also need to get familiar with GraphQL termilogies, and the framework
they are using.
...
| Intergration           | Easy to integrate with external services                           | Limited support from external service unless `/graphql` is supported | Not easy to integrate with external services |
