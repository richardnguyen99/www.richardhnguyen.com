...
sender and says "Hey, I can only handle this much data at a time. Can you please match with it?". The sender will then adjust the sending rate to match with the receiver's receiving rate.

Initially, the receive window is set to the receiver's buffer size:
